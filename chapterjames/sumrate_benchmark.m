...
X = zeros(N,M);
Rsum_non0 = zeros(Nnumx,observe_L);
...
SUMR_NON0 = zeros(observe_L,1);
SUMR_NON = zeros(observe_L,1);
X_cell = cell(Nnumx,observe_L);
X_max_non0 = cell(1,observe_L);
X_max_non = cell(1,observe_L);
...
%             R_op_cell{x,i_Dc} = R_op;
    ...
    X_max_non0{i_Dc} = cell2mat(X_cell(LOC_non0(i_Dc),i_Dc));
    [SUMR_NON(i_Dc),LOC_non(i_Dc)] = max(Rsum_non(:,i_Dc));
    X_max_non{i_Dc} = cell2mat(X_cell(LOC_non(i_Dc),i_Dc));
%     [SUMR_OP(i_Dc),loc_op(i_Dc)] = max(Rsum_op1(:,i_Dc));
end

save('Prd_nonop_N8_enum70.mat')

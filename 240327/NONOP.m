function [SUMR_NON,R_non0,loc_x,Numx2,MAX_X ]=NONOP(P_tot,i_iter,H,X_cell,Numx0,rate_thred,intfr_thred,sigma)
%% system parameters
...
Temp_intfr0 = zeros(N,M);
    ...
    for t = 1:M
        if i_iter == 1
%            P = sqrt(P_tot/M)*(X1(:,t)*X1(:,t)');
           P = sqrt(P_tot/M)*(ones(N,1)*ones(N,1)');
        else
        ...
        for n1 = 1:N
            ...
            R_non0(n1,t) = B*log2(1 + X1(n1,t)*((abs(H(:,n1)'*P(:,n1)))^2)/(intfr0(n1,t)+sigma));  
        end
    end

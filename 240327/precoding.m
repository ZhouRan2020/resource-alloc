function [W_hat,P1,P2,Rsum_non,Rsum_op1] = precoding(N,M,H_cell,B,X_exhau)
...
%                clus_list{index_loc,size_clus} = index_act{n1};
    ...
    Rsum_non(j1-181,k) = sum(R_non(:));
    R_non_cell{j1-181,k} = R_non;
 ...
 for ii = 1:length(rate_thred)   
    ...
    Rsum_op1(j1-181,k) = sum(R_op(:));
    R_op_cell{j1-181,k} = R_op;
 end
 end
end

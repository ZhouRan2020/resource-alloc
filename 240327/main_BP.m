...
demand1 = zeros(Nnumx,observe_L);
demand2 = zeros(Nnumx,observe_L);
demand3 = zeros(Nnumx,observe_L);
for i_Dc = 1:observe_L
    for x = 1:Nnumx
        ...
        intfr = zeros(N,M);    
            for t = 1:M
                W_ini = ones(N,N);
                P_ini0 = (P_tot(i_Dc)/(M*K))*diag(X(:,t));
                for n1 = 1:N
                    for n2 = 1:N 
                        if n2 ~= n1
            ...
            demand1(x,i_Dc) = min(R_non(find(R_non~=0)));
            demand2(x,i_Dc) = mean(R_non(find(R_non~=0)));
            demand3(x,i_Dc) = max(R_non(find(R_non~=0)));    
    %% sum rate of beamforming case  
            W_ini = ones(N*N,N*N);
            P_ini1 = 0.1*ones(N*N,N*N); 
            P_ini2 = 0.1*ones(N*N,N*N);
            P_ini3 = 0.1*ones(N*N,N*N);
            P_ini4 = 0.1*ones(N*N,N*N);
    %         P_ini5 = ones(N*N,N*N);
            status = 1;
            for itr = 1
                [P1,P2,P3,P4,Rsum_op,R_op,status] = beamforming(N,M,K,H,B,W_ini,X,P_tot(i_Dc),P_ini1,P_ini2,P_ini3,P_ini4,rate_thred(1));
                if status == 0                  
                   break
                end
            end
